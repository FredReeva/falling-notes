...
export class Transform{
    ...
    applyTransformations(){
        ...
        for(let i=0; i<this.piece.segments.length; i++){
            ...
            if(i==0){
                this.piece.segments[i] = new this.piece.segments[i].transform.method(this.piece, i)
                motif = this.piece.segments[i]
            }else{
                this.piece.segments[i] = new this.piece.segments[i].transform.method(this.piece, i, motif)

                let prevChord = (this.piece.segments[i-1].chord.tonic + this.piece.segments[i-1].chord.color)
                let currChord = (this.piece.segments[i].chord.tonic + this.piece.segments[i].chord.color)
    ...
    lastNoteAdjust(){
        ...
        for(let i=0; i<this.piece.segments[lastSegmentIndex].objects.length; i++){
        ...
        let lastObj = this.piece.segments[lastSegmentIndex].objects[lastObjectIndex]
        if(!lastObj){
        ...
        this.piece.segments[lastSegmentIndex].objects[lastObjectIndex].pitch = lastPieceNote
    ...
    pickTransformations(){
        ...
        for(let i=0; i<this.piece.segments.length; i++){
            ...
            if((this.piece.segments[i-1].chord.tonic + this.piece.segments[i-1].chord.color) != (this.piece.segments[i].chord.tonic + this.piece.segments[i].chord.color)){
            ...
            //Repetiton
            if(
                (this.piece.segments[i].directionality == motif.directionality)
                && ((this.piece.segments[i].chord.tonic + this.piece.segments[i].chord.color) == (motif.chord.tonic + motif.chord.color))
            ...
            //Retrogradation
            if(
                !['0', motif.directionality].includes(this.piece.segments[i].directionality)
            ) choices.push(Retrogradation)
    
            
            //PitchVariation
            if(
                ['+', '-', '0'].includes(this.piece.segments[i].directionality)
            ) choices.push(PitchVariation)

            
            //TemporalVariation
            if(
                [motif.directionality].includes(this.piece.segments[i].directionality)
            ...
            if(this.piece.segments[i].transform.method == Contrast) motif = this.piece.segments[i]
        }
